...
use crate::models::pool::{
    CpmmState, DexProtocol, Pool, PoolData, PoolId, PoolMetadata, PoolStatus, PoolToken, PoolType,
    PubkeyBytes,
};
        ...
        loop {
            ...
            for item in page.data {
                ...
                let (pool_type, pool_data) = match item.pool_type.as_str() {
                    ...
                    // Hydration via RPC `getMultipleAccounts` is required before quoting.
                    // Until that infrastructure exists, skip these pools since they
                    // cannot be priced by the optimizer (sqrt_price_x64 is missing).
                    // -------------------------------------------------------
                    "Concentrated" => {
                        log::debug!(
                            "RaydiumAdapter: Skipping CLMM pool {} (needs RPC hydration)",
                            item.id
                        );
                        continue;
                    }
            ...
            {
                anyhow::bail!("Raydium API returned repeated pagination cursor");
            }
